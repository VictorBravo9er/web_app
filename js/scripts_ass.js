const content = document.getElementById("display-section")
const dispContnet = "display-content";
/**
* TODO: Implement a function that clears all the content
* prior to generating new random content
*/
function clearAll() {
    const child = document.getElementById(dispContnet);
    if (child)
...
function showRiddle() {
  // Value should be in format: { question: '', answer: '' }
    ...
    // <button class="reveal-btn" onclick="revealAnswers()">reveal answer</button>
    const riddle_section = document.createElement("div");
    riddle_section.id = dispContnet;
    
    const riddle = document.createElement('p');
    riddle.textContent = randomRiddle.question;

    riddle_section.appendChild(riddle);

    const button = document.createElement('button');
    button.textContent = "Reveal Riddle Answer";
    button.id = "btn_reveal";
    button.className = 'reveal-btn';
    button.onclick = () => {revealAnswers(randomRiddle.answer)};

    riddle_section.appendChild(button);

    content.appendChild(riddle_section);
}
...
*/
function revealAnswers(answer) {
    const con_ = document.getElementById(dispContnet)
    con_.removeChild(document.getElementById("btn_reveal"));

    const answered = document.createElement('p');
    answered.textContent = 'The answer is ' + answer;

    con_.appendChild(answered);
